    ...
    def forward(
        ...
        **kwargs,
        ...
        if "labels" in qwen_inputs:
            old_labels = qwen_inputs["labels"]
            padding_labels = torch.full((batch_size, S * P), IGNORE_INDEX, device=device, dtype=old_labels.dtype)
            new_labels = torch.cat([padding_labels, old_labels], dim=1)
            qwen_inputs["labels"] = new_labels
        # Update dictionary
